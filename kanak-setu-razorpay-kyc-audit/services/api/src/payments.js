...
router.post('/create-order', async (req, res) => {
  ...
  if(mode === 'RAZORPAY'){
    ...
    try{
      ...
      return res.status(201).json({ gateway: 'RAZORPAY', order: rec.rows[0], razorpay: { orderId: data.id, keyId } });
    }catch(e){
...
router.post('/verify', async (req, res) => {
  ...
  const p = await query('SELECT * FROM payments WHERE order_id = (SELECT receipt FROM jsonb_populate_record(NULL::record, json_build_object('receipt', $1)) )', [razorpay_order_id]);
  // Fallback: try by pg_ref or store mapping elsewhere
  const byOrder = await query('SELECT * FROM payments WHERE order_id=$1', [razorpay_order_id])  // we used our own order_id as receipt
  const pay = byOrder.rows[0];
  ...
  await query('UPDATE payments SET status=$1, pg_ref=$2 WHERE id=$3', ['SUCCESS', razorpay_payment_id, pay.id]);
  const { rows } = await query(
...
router.post('/webhook', express.raw({ type: '*/*' }), async (req, res) => {
  try{
    ...
    // Optionally process event types (payment.captured) here by mapping receipt → order_id
